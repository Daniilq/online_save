...
#define SERVO_PIN 12      // пин сервомашинки
...
void babulka(){         // Датчик дальномера 
  distanceF();
  if(distanc <= 30 && distanc > 0){
    // резкий тормоз
...
int16_t traffic_light_value = 22;
int16_t prew_value;
int16_t this_value;
void roadTraffic(){ // функция в которой считываем сигналы светофора 
  if(Serial1.available()){ // считываем значение 
    this_value = Serial1.read(); // выводим значение как траффик...
    if(this_value == prew_value){
      traffic_light_value = this_value;
    }
    else{traffic_light_value = 22;}
    prew_value = this_value;
  }
...
void sturman(){
  ...
  if(traffic_light_value == 0 || traffic_light_value == 1 || traffic_light_value == 3 || traffic_light_value == 4){
    ...
    while(1){
      roadTraffic();
      if(traffic_light_value == 2){break;}
    }
  }
}
