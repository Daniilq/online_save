...
#define SERVO_PIN 12
#define ZERO_POS 96				//среднее положение сервы
#define MAX_SERVO_ANGLE 40		//максимальный yгол поворот(по умолчанию 40)

#define MIN_MOTOR_SPEED 120		// минимальная скорость 
#define MAX_MOTOR_SPEED 140		// максимальная скорость 
...
int16_t sensor_correction[TOTAL_SENSORS]={0, -2, -30, -1, -2, -2, 0, -2, -2, -10, -1};  // массив со значениями коррекции датчиков, для выравнивания их значений между собой
// 53,140,159,134,442,450,433,103,171,136,80,
// 0, -70, -89, -65, 0, 0, 0, -33, -100, -66, -10
...
int16_t lineCheck(int _sens_type = 0){ // проверяет наличие черной линии
	...
	for(int i=0; i<TOTAL_SENSORS; i++){
	    if((sensor_a[i] >= (black_color_val - ((black_color_val - white_color_val) / 4))) && (sensor_a[i] >= (white_color_val * 3))){ //ошибка
	    	sensor[i] = 1; // черный 
	    }
	    else{
	    	sensor[i] = 0; // белый 
...
inline void voditelWithoutPID(){ // функция которая отвечает за езду машины
	...
	}else if(line_vector_val < 0){
		_required_motor_speed = map(_line_veexp_2, -5, -11, MAX_MOTOR_SPEED, MIN_MOTOR_SPEED);
	}
	else { _required_motor_speed = MAX_MOTOR_SPEED;}
	...
	if(watch_track_val != 22){ // заставляем машину ехать когда есть линия/стоять когда нет линии
		// if (left_black >= 1 && (lr_sum != 10 && line_vector_val ))
